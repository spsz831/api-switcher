...
describe('add service', () => {
  ...
  it('add 明文 key 时会迁移到 runtime secret vault，并在落盘 profile 中仅保留引用', async () => {
    const result = await new AddService(
      {
        add: async () => undefined,
      } as any,
      {
        get: () => ({
          validate: async (profile: Profile) => {
            const reference = profile.apply.auth_reference as string | undefined
            expect(reference).toMatch(/^vault:\/\/api-switcher\//)
            expect(defaultSecretReferenceResolver.resolve(reference ?? '').status).toBe('resolved')
          ...
          preview: async () => ({
            platform: 'claude',
            profileId: 'claude-vault-profile',
            targetFiles: [],
            effectiveFields: [],
            storedOnlyFields: [],
            diffSummary: [],
    ...
    expect(result.ok).toBe(true)
    expect(result.data?.profile.source).toEqual({
      token: 'sk-vault-123456',
      baseURL: 'https://gateway.example.com/api',
    })
    expect(result.data?.profile.apply).toEqual({
      ANTHROPIC_AUTH_TOKEN: 'sk-vault-123456',
      ANTHROPIC_BASE_URL: 'https://gateway.example.com/api',
    })
    expect(result.data?.secretMigration).toEqual({
      mode: 'runtime-vault',
