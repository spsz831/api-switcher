...
describe('docs consistency', () => {
  ...
  it('README / schema 文档对 current/list platformSummary contract 保持一致', () => {
    ...
    expect(publicJsonSchemaDoc).toContain('推荐消费顺序：先读 `summary.platformStats[]` 拿平台级聚合')
    expect(publicJsonSchemaDoc).toContain('推荐消费顺序：先读 `summary.platformStats[]` 做平台分组')
    expect(publicJsonSchemaDoc).toContain('推荐消费顺序：先读 `summary.platformStats[]` 看平台级通过/限制聚合')
    expect(publicJsonSchemaDoc).toContain('推荐消费顺序：先读 `summary.platformStats[]` 看平台级聚合')
  ...
  it('README / schema 文档对 import preview/apply platformSummary contract 保持一致', () => {
    ...
    expect(publicJsonSchemaDoc).toContain('`platformSummary`、`summary.platformStats`、`scopePolicy`、`preview`、`backupId`、`changedFiles`')
  ...
  it('README / schema 文档对 use/rollback platformSummary contract 保持一致', () => {
    ...
    expect(readme).toContain('`use --json` 成功时除了 `scopeCapabilities` 与 `scopeAvailability`，还会返回 `platformSummary`。同时，`data.summary.platformStats[]`')
    expect(readme).toContain('`rollback --json` 成功时也会返回 `platformSummary`。同时，`data.summary.platformStats[]`')
    expect(publicJsonSchemaDoc).toContain('`data.summary.platformStats[]` 是成功态的单平台聚合入口')
    expect(publicJsonSchemaDoc).toContain('`platformSummary`、`summary.platformStats`、`scopeCapabilities`、`scopeAvailability`、`changedFiles`、`backupId`')
    ...
    expect(publicJsonSchemaDoc).toContain('失败样例重点看 `CONFIRMATION_REQUIRED`')
  ...
  it('schema 文档为 preview/import apply 保留成功与失败样例标题，避免顶部导航指向后正文仍不对称', () => {
    ...
    expect(publicJsonSchemaDoc).toContain('### import apply --json')
    expect(readme).toContain('`import apply --json` 成功态也会在 `data.summary.platformStats[]` 中提供单平台聚合入口')
    expect(publicJsonSchemaDoc).toContain('`summary.platformStats[]` 是 success payload 上的单平台聚合入口')
    expect(publicJsonSchemaDoc).toContain('失败样例重点看 `CONFIRMATION_REQUIRED` 或 scope unavailable 类失败')
    expect(publicJsonSchemaDoc).toContain('"action": "import-apply"')
    expect(publicJsonSchemaDoc).toContain('"code": "CONFIRMATION_REQUIRED"')
  })
  ...
  it('README / schema 文档对 validate/export platformSummary contract 保持一致', () => {
    ...
    expect(publicJsonSchemaDoc).toContain('`summary.platformStats[]` 则提供当前校验批次的 platform-aware 聚合')
    expect(publicJsonSchemaDoc).toContain('`summary.platformStats[]` 则把当前导出批次按平台聚合')
