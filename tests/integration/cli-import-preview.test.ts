...
describe('cli import preview integration', () => {
  ...
  it('import --json 非法 JSON 文件返回 IMPORT_SOURCE_INVALID 结构化错误', async () => {
    const importFile = path.join(context.runtimeDir, 'invalid-json.txt')
  ...
  it('import --json schemaVersion 不受支持返回 IMPORT_UNSUPPORTED_SCHEMA 结构化错误', async () => {
    const importFile = path.join(context.runtimeDir, 'unsupported-schema.json')
    await fs.writeFile(importFile, JSON.stringify({
      schemaVersion: '0000-01-01.unknown.v0',
      ok: true,
      action: 'export',
      data: { profiles: [] },
    }), 'utf8')
  ...
  it('import --json 包含未注册平台 profile 返回 ADAPTER_NOT_REGISTERED 结构化错误', async () => {
    const importFile = path.join(context.runtimeDir, 'unsupported-platform.json')
    await writeImportSourceFile(importFile, [
      {
        profile: {
          id: 'openai-prod',
