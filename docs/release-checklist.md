...
- schema 目录：`schema --json` 仍暴露三条 `consumerProfiles`，且 `bestEntryAction` 分别保持 `current / import / preview`；两条只读画像的 `defaultConsumerFlowId` 继续指向对应 `consumerFlow[]`；`schema --json --consumer-profile readonly-import-batch` 只裁剪 `consumerProfiles[]`，不会裁剪 `actions[]` 或完整 `schema`
